    ...
    FRotator Rotation, FVector2D SizeInMeters, bool bClearBeforeAdd)
{
		...
		if (Splash)
		{
            ...
            if (Texture !=nullptr)
            {
                Desc.LoadingTextureFromPath = Texture;
                Desc.SplashQuadSizeInMeters = SizeInMeters;
                Desc.SplashTransformInMeters = FTransform(Rotation, TranslationInMeters);
                Desc.SplashTexturePath = Texture->GetPathName();
                Splash->AddPXRSplashLayers(Desc);
                return true;
            }
		    else
		    {
