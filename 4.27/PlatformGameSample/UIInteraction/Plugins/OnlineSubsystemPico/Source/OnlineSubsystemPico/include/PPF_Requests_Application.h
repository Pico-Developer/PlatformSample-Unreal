...
/// @brief Requests version information, including the currently installed and latest
/// available version name and version code.
///
/// A message with type ::ppfMessage_Application_GetVersion will be generated in response.
///
/// First call ::ppf_Message_IsError() to check if an error occurred.
///
/// If no error occurred, the message will contain a payload of type ::ppfApplicationVersionHandle.
/// Extract the payload from the message handle with ::ppf_Message_GetApplicationVersion().
...
PPF_PUBLIC_FUNCTION(ppfRequest) ppf_Application_LaunchOtherApp(const char *appID, ppfApplicationOptionsHandle deeplink_options);
