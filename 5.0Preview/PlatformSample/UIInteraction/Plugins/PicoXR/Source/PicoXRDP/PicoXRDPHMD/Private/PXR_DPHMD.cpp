...
void FPicoDirectPreviewHMD::AdjustViewRect(int32 StereoPass, int32& X, int32& Y, uint32& SizeX, uint32& SizeY) const
#else
...
#if ENGINE_MAJOR_VERSION>4
bool FPicoDirectPreviewHMD::GetRelativeEyePose(int32 DeviceId, int32 Eye, FQuat& OutOrientation, FVector& OutPosition)
#else
...
#if ENGINE_MAJOR_VERSION>4
	if (DeviceId == IXRTrackingSystem::HMDDeviceId && (Eye == eSSE_LEFT_EYE || Eye == eSSE_RIGHT_EYE))
	{
		OutPosition = FVector(0, (Eye == eSSE_LEFT_EYE ? -.5 : .5) * 0.064f * GetWorldToMetersScale(), 0);
		return true;
	}
#else
...
#if ENGINE_MAJOR_VERSION>4
void FPicoDirectPreviewHMD::CalculateStereoViewOffset(int32 StereoPassType, FRotator& ViewRotation, const float WorldToMeters, FVector& ViewLocation)
#else
...
#if ENGINE_MAJOR_VERSION>4
FMatrix FPicoDirectPreviewHMD::GetStereoProjectionMatrix(int32 StereoPassType) const
{
	check(IsStereoEnabled() || IsHeadTrackingEnforced());
	
	FPicoXRFrustumDP Frustum = (StereoPassType == eSSE_LEFT_EYE) ? LeftFrustum : RightFrustum;
...
#if ENGINE_MAJOR_VERSION>4
void FPicoDirectPreviewHMD::GetEyeRenderParams_RenderThread(const FHeadMountedDisplayPassContext& Context, FVector2D& EyeToSrcUVScaleValue, FVector2D& EyeToSrcUVOffsetValue) const
{
...
#endif
{
	...
	{
		FRHIResourceCreateInfo CreateInfo(TEXT("DirectPreviewCreateInfo"));
