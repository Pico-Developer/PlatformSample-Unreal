...
public:
	...
	virtual bool GetRelativeEyePose(int32 DeviceId, int32 Eye, FQuat& OutOrientation, FVector& OutPosition) override;
#else
...
#if ENGINE_MAJOR_VERSION>4
	virtual void AdjustViewRect(int32 StereoPass, int32& X, int32& Y, uint32& SizeX, uint32& SizeY) const override;
	virtual void CalculateStereoViewOffset(int32 StereoPassType, FRotator& ViewRotation, const float MetersToWorld, FVector& ViewLocation) override;
#else
...
#if ENGINE_MAJOR_VERSION>4
	virtual FMatrix GetStereoProjectionMatrix(int32 StereoPassType) const override;
